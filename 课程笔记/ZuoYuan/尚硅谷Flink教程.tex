...
吞吐量以每个时间单位的事件或操作来衡量。它是
值得注意的是，加工速度取决于加工速度
到达;低吞吐量并不一定表示性能不佳。
在流式系统中，您通常希望确保您的系统可以
处理最大的预期事件率。也就是说，你是主要的
关注确定峰值吞吐量 - 性能
当系统处于最大负载时限制。为了更好地理解
峰值吞吐量的概念，让我们考虑一个流处理
应用程序没有收到任何传入的数据，因此没有
消耗任何系统资源。当第一个事件进来时，它会
尽可能以最小延迟立即处理。对于
例如，如果您是第一个出现在咖啡店的顾客
在早上开门后，您将获得服务
立即。理想情况下，您希望此延迟保持不变
并且独立于传入事件的速率。但是，一旦我们
达到传入事件的速率，使系统资源完全
使用过，我们将不得不开始缓冲事件。在咖啡店里
例如，您可能会在午餐后看到这种情况发生。许多
人们出现在同一时间，必须排队等候。在此刻，
该系统已达到其峰值吞吐量并进一步增加
事件率只会导致更糟糕的延迟。如果系统继续
以可以处理的速率接收数据，缓冲区可能变为
不可用，数据可能会丢失。这种情况是众所周知的
作为背压，有不同的策略来处理它。
