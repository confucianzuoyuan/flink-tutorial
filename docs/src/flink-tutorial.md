...
在我们的例子中，计算结果是一个`DataStream<Tuple3<String, Long, Long>>`数据记录。每一条数据记录包含了一个用户在5秒钟的周期里面的浏览统计值。计算结果组成的数据流将会调用`print()`将计算结果写到标准输出。
...
```java
DataStream<String> stringStream = env.fromElements("hello world", "atguigu bigdata");

DataStream<Integer> intStream = env.fromElements(1, 2, 3);

DataStream<Event> stream = env
  .fromElements(
    new Event("key_1", 1L, 1000L),
    new Event("key_2", 2L, 2000L),
    new Event("key_3", 3L, 3000L),
    new Event("key_4", 4L, 4000L),
  );
```
...
```java
DataStream<String> stream = env.readTextFile(文件的绝对路径);
```
...
```java
DataStream<String> keyStream = stream.map(r -> r.key);
```
...
`keyBy()`方法接收一个参数，这个参数指定了key或者keys，有很多不同的方法来指定key。我们将在后面讲解。下面的代码声明了`id`这个字段为Event流的key。
...
public class MultiStreamTransformations {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Event> tempReadings = env
                .addSource(new EventSource());
        ...
        KeyedStream<Event, String> keyedTempReadings = tempReadings
                .keyBy(r -> r.id);
    ...
    public static class RaiseAlertFlatMap implements CoFlatMapFunction<Event, SmokeLevel, Alert> {

        private SmokeLevel smokeLevel = SmokeLevel.LOW;

        @Override
        public void flatMap1(Event tempReading, Collector<Alert> out) throws Exception {
...
下面我们介绍使用匿名函数或者匿名类的方式来指定某一个字段是key。

```java
DataStream<Event> stream = env.addSource(new EventSource());
KeyedStream<Event, String> keyedStream = stream.keyBy(r -> r.key);
```
...
DataStream<String> filteredStream = stream.filter(new FlinkFilter());
```

还可以将函数实现成匿名类

```java
DataStream<String> filteredStream = stream.filter(
...
DataStream<String> filteredStream = stream.filter(new KeywordFilter("flink"));
...
```java
DataStream<String> stream = ...
DataStream<String> filteredStream = stream.filter(r -> r.contains("flink"));
```
...
public class SinkToES {
    public static void main(String[] args) throws Exception {
        ...
        ElasticsearchSink.Builder<Event> eventBuilder = new ElasticsearchSink.Builder<>(
                httpHosts,
...
public class SinkToMySQL {
    ...
    public static class MyJDBCSink extends RichSinkFunction<Event> {
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            conn = DriverManager.getConnection(
                    "jdbc:mysql://localhost:3306/kvdatabase",
                    "username",
                    "password"
            );
            insertStmt = conn.prepareStatement("INSERT INTO kvtable (key, value) VALUES (?, ?)");
            updateStmt = conn.prepareStatement("UPDATE kvtable SET value = ? WHERE key = ?");
        }

        @Override
        public void invoke(Event event, Context context) throws Exception {
            updateStmt.setLong(1, event.value);
            updateStmt.setString(2, event.key);
            updateStmt.execute();

            if (updateStmt.getUpdateCount() == 0) {
                insertStmt.setString(1, event.key);
                insertStmt.setLong(2, event.value);
                insertStmt.execute();
            }
        }
...
.assignTimestampsAndWatermarks(
        WatermarkStrategy
                ...
                .withTimestampAssigner(new SerializableTimestampAssigner<Event>() {
                    @Override
                    public long extractTimestamp(Event r, long l) {
                        return r.timestamp;
                    }
                })
)
...
class PunctuatedAssigner extends AssignerWithPunctuatedWatermarks[Event] {
  ...
  override def checkAndGetNextWatermark(r: Event, extractedTS: Long) {
    if (r.id == "sensor_1") {
      // 抽取的时间戳 - 最大延迟时间
  ...
  // 每来一条数据就调用一次
  override def extractTimestamp(r: Event, previousTS: Long) {
    r.timestamp
  }
}
```
...
public class SensorSwitch {
    ...
    public static class SwitchProcess extends CoProcessFunction<Event, Tuple2<String, Long>, Event> {
...
class SelfCleaningTemperatureAlertFunction(val threshold: Double)
  ...
  override def processElement(
      ...
      ctx: KeyedProcessFunction
        [String, Event, (String, Double, Double)]#Context,
      out: Collector[(String, Double, Double)]): Unit = {
  ...
  override def onTimer(
      ...
      ctx: KeyedProcessFunction[String,
        Event, (String, Double, Double)]#OnTimerContext,
      out: Collector[(String, Double, Double)]): Unit = {
...
class DerbyUpsertSink extends RichSinkFunction[Event] {
  ...
  override def invoke(Event r, context: Context[_]): Unit = {
...
val dataStream: DataStream[Event] = inputStream
  .map(data => {
    val dataArray = data.split(",")
    Event(dataArray(0), dataArray(1).toLong, dataArray(2).toDouble)
  })
...
object TumblingWindowTempCount {
  def main(args: Array[String]): Unit = {
    ...
    val stream = env.addSource(new EventSource).filter(r => r.id.equals("sensor_1"))
...
HiveCatalog的设计提供了与 Hive 良好的兼容性，用户可以”开箱即用”的访问其已有的 Hive 数仓。 您不需要修改现有的 Hive Metastore，也不需要更改表的数据位置或分区。### Maven依赖
